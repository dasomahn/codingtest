...
def dfs(x_shark, y_shark, eat, graph, depth):
    global answer

    num, dirnum = graph[x_shark][y_shark]
    print(f"{depth}) shark 현재 {x_shark} {y_shark}={num}, 방향num은 {dirnum+1}")
    print(f"{depth}) 총 먹은건 {eat}, 그리고 grid는 {graph}")
    ...
    print(f"{depth}) 갈수있는 {position}들. 상어 움직인당")

    # 이동가능한 좌표가 남은 경우
    if position:
        for nx_shark, ny_shark in position:
            print(f"{depth}) 상어 {nx_shark} {ny_shark} 먹으러 출발")
            dfs(nx_shark, ny_shark, eat, graph, depth+1)
    else:
        print(f"--------------------\n{depth} 움직일 수 없음. 기존 max {answer} 현재 값 {eat}\n----------------------")
